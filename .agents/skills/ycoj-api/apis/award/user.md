...
POST `type Input={oierId:number}`, example `{"oierId":12}`. Response `type Redirect={url:string}`, example `{"url":"/home/award"}`. The server re-checks that the caller is still real-name approved with a stored `realName`, that the caller has no `oierId`, that the contestant is not bound to another account, and that the contestant’s `schools`/`latestSchool` share an exact canonical school name with the caller’s verified school (aliases count; substring matches used on GET do not). Name or school mismatch both return `AwardNameMismatchError`. Success sets `user.oierId`, `user.oierBoundAt`, and `user.ccfLevel` only after a final verification immediately before the user write. Bind and unbind for the same uid are serialized. Errors: `AwardRealnameRequiredError` (403), `AwardAlreadyBoundError` (403), `AwardOierTakenError` (403), `AwardNameMismatchError` (403), `AwardOierNotFoundError` (404). Rate limit: 5 binds per 60 seconds per user.
