...
New solutions start unreviewed (`reviewStatus: 1`, `revision: 0`). Existing solutions are migrated to approved (`2`). Lists sort featured (`3`), approved (`2`), unreviewed (`1`), rejected (`0`), then author-blocked (`-1`), with descending votes and IDs within each state before pagination. Unreviewed/rejected/blocked content and replies are collapsed by default but publicly expandable, including permalinks. Raw Markdown remains accessible under the same view permissions.
