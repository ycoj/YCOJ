...
The prompt requires Markdown structure, LaTeX math (`$...$` and `$$...$$`), and paired sample fences named ````input{x}```` and ````output{x}````; the model must return Markdown only. Configuration and permission errors remain immediate API errors. The admitting process runs the model call in-process and is the only process that ever issues the `pending` → `running` claim for that job, so a job cannot be replayed on another worker. Capacity is enforced cluster-wide and atomically against races using unique slot indexes over in-flight jobs: at most 100 live (`pending`/`running`) jobs total and at most 10 per `{ domainId, pid, uid }` owner; exceeding either returns HTTP 503 `HtmlToMarkdownCapacityError`. Settling a job (`completed` or `failed`) immediately frees its slots. Provider failures are reported by polling with a generic error message, never provider diagnostics or credentials.
