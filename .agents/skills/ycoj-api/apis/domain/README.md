...
| `/manage/realname` GET, POST | List/review real-name applications. `GET ?page=1&status=pending`; POST `{operation:"approve"|"reject"|"revoke",id:ObjectId,reason?:string}`. | HTML/redirect; super administrator (`PRIV_ALL`) only. |
