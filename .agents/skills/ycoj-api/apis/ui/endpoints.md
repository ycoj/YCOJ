...
## `GET /ui/nav`
Description: retrieve navigation available in the active domain. Request `type Query={}`, example `GET /ui/nav`. Response `type Nav={navItems:unknown[];user:Record<string,unknown>&{realnameStatus:"none"|"pending"|"approved"|"rejected";realnameSubmittedAt?:string|null;modType:"su"|"mod"|null}}`, example `{"navItems":[{"name":"home","url":"/"}],"user":{"_id":12,"uname":"alice","realnameStatus":"approved","realnameSubmittedAt":"2026-09-01T00:00:00.000Z","modType":null}}`; filtered by current user/domain visibility. `user` is the caller’s public user fields plus `realnameStatus` (normalized; missing/unknown becomes `"none"`) and `modType`, plus `realnameSubmittedAt` (the first application timestamp serialized as an ISO-8601 string; absent or null before submission). Pending or rejected users remain within grace while the current time is strictly before this timestamp plus seven days. Updates and resubmissions do not reset it. This field is explicitly included even though generic user serialization omits it.
...
Description: serve versioned UI-default lazy/resource assets and UI-next plugin assets. Request `type AssetPath={version:string;name:string}`, examples `/lazy/abc/main.js`, `/resource/abc/logo.svg`, `/plugins/abc/chunk.js`. Response `type Asset=Binary`, example JavaScript bytes `export default {};`; content type/cache headers come from asset handler. Public, including unverified logged-in users (`skipRealnameCheck`; does not throw `RealnameRequiredError`). `name` is filename-validated and absent versions/files are not found.
