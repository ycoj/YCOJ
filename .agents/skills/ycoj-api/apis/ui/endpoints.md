...
## `GET /ui/languages`
Description: list configured languages, optionally constrained by problem. Request `type Query={pid?:number}`, example `GET /ui/languages?pid=1000`. Response `type Languages={languages:Record<string,{display:string;versions:Array<{display:string;name:string}>}>}`, example `{"languages":{"cc":{"display":"C/C++","versions":[{"display":"C++17","name":"cc.cc17"}]}}}`; problem id is validated when supplied.
