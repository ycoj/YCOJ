...
| `/manage/realname` | GET list of the latest application per user, with optional `page`, `status`, and case-insensitive username substring `uname` filter; POST `approve` / `reject` / `revoke` | Super administrator (`PRIV_ALL`) |
