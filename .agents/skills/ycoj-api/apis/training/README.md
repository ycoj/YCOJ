...
* `GET /training/create` or `/training/:tid/edit` renders `training_edit.html` with `{ page_name, pdict }`; for an existing training, `pdict` contains every problem document referenced by its DAG (and is `{}` when creating a training).
