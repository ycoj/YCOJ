...
`GET /contest/:tid/management` -> management HTML/PJAX. Multipart operations `uploadFile` (`filename`, `type?:public|private`, file), `deleteFiles` (`files[]`, type), and JSON `setScore` (`pid:PositiveInt,score:PositiveInt`) redirect/back or return operation JSON. `GET/POST /contest/:tid/bulk-submit` is documented in [bulk-submit.md](bulk-submit.md): GET renders the mapping form; POST uploads a contestant zip (`mapping` JSON of website pid → zip folder name, optional C++ `lang`, optional `dryrun`). Vuser and judge-record creation occurs only outside dry-run mode; dryrun=true reports planned actions without mutating contest state. `GET /contest/:tid/clarification` renders clarification management; POST `clarification` accepts `{content,did?:ObjectId,subject?:Int}`.
