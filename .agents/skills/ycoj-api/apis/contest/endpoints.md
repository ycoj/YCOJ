...
Description: upload a zip of contestant C++ sources and map website pids to zip folder names. When dryrun=false, create/reuse a vuser per contestant, attend the contest, insert records, and enqueue judge tasks. When dryrun=true, report planned actions without mutating contest state. Allowed after the contest has started, including after it ends. Request: `type B={mapping:string|Record<number,string>;lang?:string;dryrun?:boolean;file:Blob}`; multipart `POST /contest/665f.../bulk-submit` with `file=@weekly.zip&mapping={"1001":"apple"}&lang=cc.cc14&dryrun=false`. Response: `type R={dryrun:boolean;lang:string;users:{uname:string;uid:number;created:boolean}[];submitted:{uname:string;uid:number;pid:number;rid?:ObjectId}[];skipped:{uname:string;problem:string;reason:string}[]}`; `{ "dryrun":false,"lang":"cc.cc14","users":[{"uname":"alice","uid":-1000,"created":true}],"submitted":[{"uname":"alice","uid":-1000,"pid":1001,"rid":"665f..."}],"skipped":[] }`. Requires contest owner/maintainer or `PERM_EDIT_CONTEST`. Not-started contests return `ContestNotLiveError`. Full contract: [bulk-submit.md](bulk-submit.md).
