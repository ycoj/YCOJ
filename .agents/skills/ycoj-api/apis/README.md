...
This is the routing index for the backend contracts present at source revision `9ee423517944` (2026-08-28). It covers all 135 literal HTTP route registrations, five literal connection registrations, and the indirectly registered `/api/:op` HTTP/connection transport. Routes from optional packages or conditional configuration are documented but exist at runtime only when their package or feature is enabled.
...
| `/home/realname`, `/home/realname/result`, `/manage/realname` | [Real-name](realname/README.md) | User submission/result plus super-admin review; unverified users are blocked from other feature routes |
