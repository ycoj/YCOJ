...
function isValidConfig(config) {
    ...
    const processCount = config.type === 'communication' ? config.num_processes : 1;
    const time = processCount * Math.sum(...config.subtasks.flatMap((subtask) => subtask.cases.map((c) => c.time)));
