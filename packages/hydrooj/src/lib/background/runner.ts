import { BackgroundTaskModel, backgroundTaskModel, type BackgroundTaskOwner } from '../../model/backgroundTask';
...
export class BackgroundTaskService {
    ...
    async submit(type: string, owner: BackgroundTaskOwner, payload: any) { const d = this.require(type); const id = await this.store.submit(type, owner, payload); if (!id) return null; const p = this.run(id, d).finally(() => this.active.delete(p)); this.active.add(p); return { jobId: id, status: 'pending' as const }; }
    ...
    private async run(id: string, d: BackgroundTaskDefinition) { if (!await this.store.claim(id, d.type)) return; const c = new AbortController(); this.controllers.set(id, c); const timer = setTimeout(() => c.abort(), d.timeoutMs); timer.unref(); try { const result = await d.run((await this.store.coll.findOne({ jobId: id, type: d.type }))?.payload, c.signal); if (c.signal.aborted) throw new Error('timeout'); await this.store.finish(id, d.type, { result }, d.retentionMs); } catch { await this.store.finish(id, d.type, { error: c.signal.aborted ? (d.timeoutError ?? 'Background task timed out.') : 'Background task failed.' }, d.retentionMs); } finally { clearTimeout(timer); this.controllers.delete(id); } }
}
