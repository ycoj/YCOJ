...
import { BackgroundTaskModel, backgroundTaskModel, ensureBackgroundTaskIndexes, BACKGROUND_TASK_INDEXES, type BackgroundTaskDoc, type BackgroundTaskOwner } from './backgroundTask';
...
export class HtmlToMarkdownJobModel {
    ...
    finish(id: string, result: { markdown?: string, error?: string }, retention: number, now?: Date) { return this.base().finish(id, TYPE, { result: result.markdown ? { markdown: result.markdown } : undefined, error: result.error }, retention, now); }
    ...
    async view(id: string, owner: HtmlToMarkdownJobOwner, o: { timeoutMs: number, retentionMs: number, now?: Date }) { const d = await this.base().view(id, TYPE, owner, o.timeoutMs, o.retentionMs); if (d?.error === 'Background task timed out.') d.error = HTML_TO_MARKDOWN_TIMEOUT_MESSAGE; return d ? asView(d) : null; }
