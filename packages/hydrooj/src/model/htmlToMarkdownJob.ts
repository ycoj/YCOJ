...
export class HtmlToMarkdownJobModel {
    ...
    markFailed(id: string, error: string, retention: number, now?: Date) { return this.coll.updateOne({ jobId: id, type: TYPE, status: { $in: ['pending', 'running'] } }, { $set: { status: 'failed', error, finishedAt: now ?? new Date(), expiresAt: new Date((now ?? new Date()).getTime() + retention) }, $unset: { ownerSlot: '', globalSlot: '', result: '' } }); }
