...
// Slot capacity is enforced atomically by unique indexes over slot numbers. Both slot indexes are
// partial on $exists so that a doc stops occupying its slot the moment it reaches a terminal
// status and the fields are unset, instead of holding capacity until the TTL expiry.
export const BACKGROUND_TASK_INDEXES: IndexDescription[] = [
    { key: { jobId: 1 }, name: 'jobId', unique: true },
    {
        ...
        partialFilterExpression: { ownerSlot: { $exists: true } },
    },
    {
        key: { type: 1, globalSlot: 1 }, name: 'globalSlot', unique: true,
        partialFilterExpression: { globalSlot: { $exists: true } },
    },
...
const free = (used: Set<number | undefined>, cap: number) => { for (let i = 0; i < cap; i++) if (!used.has(i)) return i; return -1; };
export class BackgroundTaskModel {
    ...
    async submit(type: string, owner: BackgroundTaskOwner, payload?: any, now = new Date()) {
        ...
        for (let i = 0; i < this.capacity + this.capacityPerOwner; i++) {
            const active = { status: { $in: ['pending', 'running'] as BackgroundTaskStatus[] } };
            const [a, g] = await Promise.all([
                this.coll.find({ ...this.filter(type, owner, now), ...active }, { projection: { ownerSlot: 1 } }).toArray(),
                this.coll.find({ ...this.filter(type, undefined, now), ...active }, { projection: { globalSlot: 1 } }).toArray(),
            ]);
